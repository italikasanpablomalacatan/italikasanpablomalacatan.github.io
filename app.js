...
function buildCats(){let c={Todas:motos.length};motos.forEach(m=>c[m.categoria]=(c[m.categoria]||0)+1);cats.innerHTML=Object.entries(c).map(([k,v])=>`<button class="cat ${k==active?"active":""}" data-cat="${k}">${k} · ${v}</button>`).join("");$$(".cat").forEach(b=>b.onclick=()=>{active=b.dataset.cat;buildCats();render()})}
function list(){let s=search.value.toLowerCase().trim();let l=motos.filter(m=>(active=="Todas"||m.categoria==active)&&`${m.nombre} ${m.detalle} ${m.categoria} ${m.sku} ${m.codigo}`.toLowerCase().includes(s));if(sort.value=="priceAsc")l.sort((a,b)=>a.precio-b.precio);if(sort.value=="priceDesc")l.sort((a,b)=>b.precio-a.precio);if(sort.value=="nameAsc")l.sort((a,b)=>a.nombre.localeCompare(b.nombre));return l}
function render(){grid.innerHTML="";list().forEach(m=>{let c=document.createElement("article");c.className="card";c.innerHTML=`<span class="tag">${m.categoria}</span><div class="photo"><img loading="lazy"></div><h3>${m.nombre}</h3><p class="detail">${m.detalle}</p><div class="price">${priceHtml(m)}</div><button class="interest">Me interesa esto</button>`;img(c.querySelector("img"),m.img,m.nombre);c.onclick=()=>openMoto(m);c.querySelector(".interest").onclick=e=>{e.stopPropagation();selectedMoto=m;openPayment()};grid.appendChild(c)})}
